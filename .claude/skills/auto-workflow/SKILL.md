...
ステップ2.6で特定した「ユーザーアクション必要」項目がある場合、**PR作成前に Notion の Task DB にタスクとして起票**する。起票後の Notion URL を 6.5 の PR 本文に含めるため、このステップを先に実行する。

**起票条件**: ユーザーアクション項目が1つ以上ある場合のみ実行。ない場合はスキップして 6.5 へ。
...
**ステップ6.4で起票したタスクがある場合、Notion URL をリンク付きチェックリストとして含めること。**
...
タスクが無い場合は「ユーザーアクション必要」セクション自体を省略する。
